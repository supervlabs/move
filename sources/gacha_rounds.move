...
module supervlabs::gacha_rounds {
    ...
    #[view]
    public fun get_probabilities(round_obj: address): (u64, vector<String>, vector<u64>, u64) acquires GachaRounds {
        let (round, current_round) = get_current_round(round_obj);
        let len = vector::length<GachaDropRate>(&round.numerators);
        assert!(len > 0, error::not_found(EGACHA_ROUNDS_NOT_CONFIGURED));
        ...
        vector::for_each_ref(&round.numerators, |e| {
            let drop_rate: &GachaDropRate = e;
            vector::push_back(&mut grades, e.grade);
            vector::push_back(&mut numerators, drop_rate.numerator);
        });
        (current_round, grades, numerators, round.denominator)
    }
    ...
    public fun update_probabilities(
        creator: &signer,
        round_obj: address,
        _target_round: u64,
        grades: vector<String>,
        ...
        let grs = borrow_global_mut<GachaRounds>(round_obj);
        assert!(grs.creator == signer::address_of(creator),
            error::invalid_argument(ENOT_AUTHORIZED_OPERATION));
        grs.rounds = vector[ round ];
        return grs.current_round
    }
    ...
    #[view]
    public fun exists_at(round_obj: address): bool {
        exists<GachaRounds>(round_obj)
    }
}